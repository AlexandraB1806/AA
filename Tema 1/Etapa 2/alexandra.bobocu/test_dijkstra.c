...
void free_matrix_dijkstra(int **mat, int n)
{
    for (int i = 0; i < n; i++)
        free(mat[i]);
    free(mat);
...
int main()
{
	...
	for (int file = 0; file < NUM_TESTS; file++) {
		...
		// Parcurg fiecare nod si il marchez ca nod sursa
		for (int vertice = 0; vertice < nodes; vertice++) {
			err = dijkstra(matrix, results, nodes, vertice);
			if (err == 1)
				break;
		}
	...
	free_matrix_dijkstra(matrix, MAX_SIZE);
	free_matrix_dijkstra(results, MAX_SIZE);
